 ...
 * 
 * @article https://suisen-kyopro.hatenablog.com/entry/combination-arbitary-mod-2
 * 
 * @task https://atcoder.jp/contests/arc012/tasks/arc012_4
 * @submission https://atcoder.jp/contests/arc012/submissions/18497791
 */
public class ArbitaryModBinom2 {
    private static final int MAX_PRIME_FACTOR_NUM = 9;

    /**
     * Calculates [C(N,0),C(N,1),...,C(N,N)] in O(NlogM/loglogM) time.
     * @param N <= 10^7
     * @param M <= 10^9
    ...
    public static long[] solve(final int N, final long M) {
        ...
        for (int i = 1; i <= N >> 1; i++) {
            ...
            long commonProd = 1;
            for (int j = 0; j < primeFactorCount; j++) {
                commonProd = ma.mul(commonProd, pow[j][index[j]]);
            }
